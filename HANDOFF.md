...
**Fase 2 LIVE (8 juli, basis = RP nabouwen, daarna samen verbeteren)**: /admin/pipeline — kanban-bord op eigen leads (KV): kolommen met aantal+totaalwaarde, kaartjes slepen (dataTransfer, getest incl. server-side statuswissel+timeline), zoeken, ondertekend-badge, login met admin-wachtwoord. Automations bij verplaatsen = bestaande changeStatus (timeline/Telegram/Klaviyo/klantmail). Fase 3 = migratie 16,7k RP-items + RP opzeggen (opzegtermijn nog vragen aan Daimy).
