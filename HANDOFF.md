...
action=configurator-prijs (route offerte-tool) + lib/offerte-tool/configurator-map.ts: variant/bediening/framekleur → centrale prijsengine (incl. KV-prijsconfig). Getest backend: alle varianten cent-exact gelijk aan v4 (S-42 solar+RAL 1670×1360 = €1.629,64 ✓). Client: "Jouw prijs" zodra exact (valt terug op schatting), sticky footer onderaan mobiel met exacte prijs + Verder-knop (visueel getest), bedragen 2 decimalen. VOLGT in de rebuild: defaults/bestseller vooringevuld, stepper met vinkjes, meetinstructies+diagram, productvisual, URL-state, beschrijving-generator (v4-teksten aan de bron). Werkwijze-mandaat Daimy: doorbouwen + altijd backend & visueel testen tot goed (memory feedback_doorbouwen_testen).
