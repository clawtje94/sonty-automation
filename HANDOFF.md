...
0d. **Alleen-Roma-verstuurd hersteld (10 juli, akkoord Daimy)**: scan wees uit dat 47 van 152 duo-klanten ALLEEN de Roma-duo kregen (Sunmaster bleef DRAFT; team koos in RP het verkeerde/nieuwste document — aanleiding: klacht Angela van Spijk 20269119/20269161). Alle 47 hebben op 10 juli de inhaalmail met beide offertes gekregen via `scripts/inhaal-sunmaster-mail.js` (Trengo Aanvragen; verstuurd-log `data/inhaal-sunmaster-verstuurd.json`, scan `data/duo-verzendstatus-2026-07-10.json`). Zelfde dag: rolluik-duo's omgezet van .XP naar .P (prijsboek p82-85, opdracht Daimy) en geërfd "Waarom Sonty"-blok uit duo's gehaald — 90 documenten herberekend, 6 met klant-akkoord overgeslagen. LET OP: fixes staan in PR #1 (branch worktree-majestic-munching-token); tot merge + pull maakt de v4-cron nieuwe duo's nog met .XP + Waarom-Sonty-blok. Angela's vervolgvraag (4 extra ramen, bekabeld + solar) staat nog open in Trengo-ticket 965041807; AI-KS-bot ligt eruit (API-tegoed op).
