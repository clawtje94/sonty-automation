...
//         O5 lege offerte → geen regels (de cron slaat hem zichtbaar over: "geen productregels"); O6 'Waarom'-/garantieblok nooit in Gripp.
...
const dims = [
  ...
  { naam: 'waarom', waarden: [{ label: 'zonder' }, { label: 'met-blok', tekst: '\n\n**Waarom dit rolluik**\nPremium kwaliteit\n\nGarantie: 5 jaar' }] },
];
...
function orakel(s) {
  ...
  return { wil: 'ok', geenWaarom: true, regels: leeg ? 0 : 2, eersteAantal: leeg ? null : s.aantal.n, eerstePrijs: leeg ? null : 1250, pct: s.korting.pct || null, status: s.status.label, nummer: 'S26-2001' };
}
function voerUit(s) {
  ...
  return { geenWaarom: !lines.some((l) => /waarom|garantie/i.test(l.description)), regels: lines.length, eersteAantal: lines[0] ? lines[0].units : null, eerstePrijs: lines[0] ? lines[0].pricePerUnit : null, pct: gd ? gd.amount : null, status: d.status, nummer: d.info.quotationNumber, melding: false };
}
function vergelijk(w, e) { return w.geenWaarom === e.geenWaarom && w.regels === e.regels && w.eersteAantal === e.eersteAantal && w.eerstePrijs === e.eerstePrijs && w.pct === e.pct && w.status === e.status && w.nummer === e.nummer; }
