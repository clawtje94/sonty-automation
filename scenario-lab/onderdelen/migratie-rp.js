...
const { rpItemNaarLead } = require('../../scripts/migreer-rp-naar-eigen.js');
...
function voerUit(s) {
  const l = rpItemNaarLead(maakItem(s), quotations(s));
  return { id: l.id, kolom: l.rpKolom, gearchiveerd: !!l.gearchiveerd, email: l.contact.email, telefoon: l.contact.telefoon, postcode: l.contact.postcode, plaats: l.contact.plaats,
...
function vergelijk(w, e) { return ['id', 'kolom', 'gearchiveerd', 'email', 'telefoon', 'postcode', 'plaats', 'rpNummer', 'offerteStatus', 'totaal', 'type', 'producten'].every((k) => w[k] === e[k]); }
