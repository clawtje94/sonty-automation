  ...
  try {
    ...
    const resp = await client.messages.create({
      ...
      messages: [{ role: 'user', content:
        ...
        `(1) past het op het laatste klantbericht qua onderwerp en persoon? Aanspreken met alléén de VOORNAAM is prima en juist — keur NOOIT af omdat de volledige/achternaam niet is gebruikt; alleen afkeuren bij een écht verkeerde persoon of een verkeerd onderwerp. (2) geen dag-fouten — "fijn weekend" mag alleen als het volgens de opgegeven huidige datum echt vrijdag(middag)/weekend is (berichttijden zijn NL-tijd). REKEN ZORGVULDIG met datums: een datum op of ná de huidige datum ligt in de TOEKOMST en is een geldige afspraakdatum (voorbeeld: is het vandaag 21 juli, dan is 23 juli gewoon overmorgen en prima voor te stellen); keur NOOIT af wegens "datum in het verleden" tenzij de datum écht vóór vandaag ligt, en bij de minste datumtwijfel: OK. Voorgestelde afspraaktijden komen uit het echte agendasysteem — beoordeel hun beschikbaarheid niet; (3) geen herhaald/gestapeld afscheid als er al afscheid is genomen; (4) geen opsmuk met het woord "zonnig" in welke vorm dan ook ("zonnige groet"/"zonnige zomer"/"zonnige zaken" → altijd afkeuren). LET OP: "Sunny" is de NAAM van de medewerker (de afzender ondertekent met "Sunny | Sonty") — dat is GEEN opsmuk en NOOIT een reden om af te keuren; verwar de naam Sunny niet met het woord "zonnig"; (5) zelfde taal als de klant; (6) geen interne info gelekt (team, notities, systemen); (7) spreekt zichzelf niet tegen t.o.v. eerder in het gesprek — MAAR een bevestiging van een zojuist uitgevoerde actie is geen tegenspraak (zie LET OP hierboven); (8) beantwoordt de vraag i.p.v. eromheen te praten; (9) PRIJSBEZWAAR NIET OMDRAAIEN: zegt de klant iets als "aardig aan de prijs", "pittig", "stevig", "niet goedkoop", "flink bedrag" of "daar schrok ik van", dan vindt hij het DUUR — keur af als het concept dat als compliment leest ("leuk dat de prijs je bevalt"), om een handtekening vraagt of de 75 euro noemt (fout bij Edwin, 27 juli); (10) SLOTVRAAG (spoed-uitzondering: staat de klant NU ergens, is hij onderweg of heeft hij acuut praktische info nodig, dan telt directe hulp zoals adres en routetip zwaarder dan een slotvraag; keur zulke hulp NOOIT af en eis er geen keuzevraag bij): loopt het verkoopgesprek nog (geen afscheid, geen klacht of servicekwestie, geen escalatie, klant stuurde meer dan alleen een duimpje of bedankje) en staat er NERGENS in het concept een concrete vraag aan de klant, keur dan af met reden "eindigt zonder slotvraag". Staat er wel ergens een vraag (ook als niet-laatste zin), dan is dat GEEN afkeurreden — uit de analyse van Daimy blijkt dat 96% van de doodgelopen gesprekken eindigde zonder vraag; (11) VALSE BELOFTES: keur af als het concept een eigen tijdsbelofte doet ("we bellen je vandaag", "je hoort zo/straks van ons", "morgen belt de planning") want de bot weet niet wanneer het team belt; alleen het vaste "binnen 5 werkdagen"-proces mag genoemd worden. Keur ook af als het concept zegt dat iets "geregeld" of "in orde gemaakt" is terwijl het alleen is doorgegeven of voorgesteld; (12) INMETEN ZONDER INDICATIE: keur af als het concept het inmeten in gang zet of aankondigt terwijl er in het hele gesprek nog geen maten zijn uitgevraagd en geen prijs of prijsindicatie is genoemd (uitzondering: service/reparatie) — eerst indicatie, dan inmeten is het vaste proces.\n` +
        ...
        `Let op bij e-mail: een klantbericht bevat vaak de oude thread als quote. Staat er een [NIEUW BERICHT]-markering, beoordeel het concept dan ALLEEN tegen dat nieuwe deel — de gequote tekst eronder is al eerder behandeld.\n` +
        `Beoordeel UITSLUITEND op de punten hierboven en verzin geen eigen eisen. Voorbeeld van een verzonnen eis (echt gebeurd, 31 juli): afkeuren omdat "de offertelink vernieuwd zou moeten zijn" na een aanpassing — een offerte houdt na een aanpassing juist DEZELFDE link, identiek is correct. Antwoord met exact "OK" of met "AFGEKEURD: <één korte concrete reden>".` }],
    });
  ...
  for (let iter = 0; iter < 9; iter++) {
    ...
    if (tekst) {
      ...
      if (!/^OK\b/i.test(oordeel)) {
        ...
        // STIJL IS GEEN VETO (casus Bianca 31-07): keurt de QA voor de tweede keer af op
        // alleen stijl (slotvraag), dan gaat het inhoudelijk goede antwoord GEWOON de deur
        // uit. Een klant met een akkoord heeft niets aan "een collega komt erop terug"
        // terwijl het echte antwoord (nieuwe prijs, link, tekenvraag) klaarligt.
        if (/slotvraag/i.test(oordeel)) {
          console.log('  QA-stijlafkeuring genegeerd na herkansing, antwoord gaat toch uit');
          return { antwoord: tekst, acties: ctx.acties, toolCalls, usage, qa: 'OK (stijlafkeuring genegeerd: ' + oordeel.slice(0, 80) + ')', klaar, opgelost };
        }
        ...
        // MAAR (casus Bianca 31-07): nooit twee keer hetzelfde wachtbericht naar de klant —
        // stond het er al, dan stil escaleren. En is er net een actie GELUKT (offerte
        // aangepast), zeg dat er dan eerlijk bij in plaats van een kaal wachtbericht.
        const alEerderGewacht = String(historie || '').includes('geen half antwoord op geven');
        if (alEerderGewacht) return { antwoord: null, acties: ctx.acties, toolCalls, usage, qa: oordeel };
        const gelukt = ctx.acties.some(a => a.type === 'offerte_aanpassen' || a.type === 'inmeet_afspraak');
        return {
          antwoord: gelukt
            ? 'De aanpassing is gelukt en staat klaar. Een collega stuurt je zo de bijgewerkte offerte met de link, dan kun je meteen bekijken of alles klopt.'
            : 'Ik wil je hier geen half antwoord op geven, dus ik leg dit even bij een collega neer. Je hoort zo snel mogelijk van ons!',
