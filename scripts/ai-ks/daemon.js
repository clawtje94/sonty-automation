...
async function planningRolVoor(t, rows) {
  ...
  const context = [
    ...
    sunnyPlant && sunnyPlant.intent === 'annuleren'
      ? `- DE KLANT WIL ZIJN INMEETAFSPRAAK ANNULEREN en JIJ handelt dit volledig af (${sunnyPlant.samenvatting || 'zie zijn bericht'}). Roep inmeet_annuleren aan met zijn letterlijke annuleringszin als citaat. Twijfel je of hij annuleren of verzetten bedoelt: vraag dat éérst kort na ("wil je hem helemaal annuleren, of liever een ander moment?"). Dit is een lopend gesprek: begin niet met "Hoi naam".`
      : sunnyPlant
