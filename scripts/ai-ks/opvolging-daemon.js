...
async function beoordeel(k, echte, context) {
  ...
  const geplandInstructie = k.gepland ? `\nLET OP: de klant zei eerder ZELF dat hij of zij er rond dit moment op terug zou komen (gepland op ${k.geplandDatum}). Dat moment is nu aangebroken en de klant heeft nog niets laten horen. Een korte, vriendelijke check-in is dan vrijwel altijd gepast; verwijs licht naar wat de klant toen zei ("je zou er dit weekend naar kijken"). Hebben WIJ toen iets beloofd (planning belt na die datum)? Reken die belofte niet als gebeurd; een check-in is dan zeker gepast.\n` : '';
