...
// Output: scratchpad/trengo-open-mails.json (met Id voor later op-gelezen-zetten).
const { chromium } = require('/Users/clawdboot/sonty/node_modules/playwright');
const fs = require('fs');
const OUT = '/private/tmp/claude-501/-Users-clawdboot/6cd82455-8977-4ee7-aa32-c9c14b2a7c8f/scratchpad/trengo-open-mails.json';
...
(async () => {
  ...
  const open = folders.find((f) => /^trengo open$/i.test(f.DisplayName));
  if (!open) { console.log('MAP NIET GEVONDEN'); process.exit(1); }
  const mRes = await page.request.get(
    base + `/MailFolders/${open.Id}/messages?$filter=IsRead eq false&$top=50&$select=Subject,From,ReceivedDateTime,Body,IsRead&$orderby=ReceivedDateTime asc`,
    { headers: H }
  );
