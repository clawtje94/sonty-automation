...
const SNAPSHOT = path.join(UIT, 'nulmeting.json');
...
function meet({ markupOverride } = {}) {
  ...
  return {
    ...
    corpus: {
      ...
      uniekePrijsvragen: corpus.unieke.length,
...
function vergelijk(oud, nieuw, factor) {
  ...
  for (const k of alle) {
    ...
    uit.vergeleken++;
    const verwacht = a.v4 === null ? null : factor ? Math.round(a.v4 * factor * 100) / 100 : a.v4;
    // 1 cent speling bij een factor: afronding van boekprijs × opslag kan een cent schelen.
    const speling = factor ? 0.011 : 0;
    const okV4 = verwacht === null ? b.v4 === null : b.v4 !== null && Math.abs(b.v4 - verwacht) <= speling;
    const okBot = b.bot === b.v4; // bot en v4 horen altijd hetzelfde te zeggen
    if (okV4 && okBot) { uit.gelijk++; continue; }
    uit.afwijkend.push({
      k, kop: a.kop,
      oudV4: a.v4, nieuwV4: b.v4, verwacht,
      nieuwBot: b.bot,
      soort: !okV4 ? 'prijs wijkt af' : 'bot en v4 zeggen iets anders',
    });
  }
  return uit;
}
...
function toonMeting(m) {
  ...
  console.log(`  unieke prijsvragen       ${c.uniekePrijsvragen}`);
...
function zelftest() {
  ...
  const sun = Object.entries(normaal.prijzen).filter(([, p]) => p.soort === 'sunmaster' && p.v4 !== null);
  const d = vergelijk(normaal, verstoord, null);
  const gezien = d.afwijkend.filter((a) => String(a.k).startsWith('sunmaster')).length;

  console.log(`  Sunmaster-prijzen in de meting        ${sun.length}`);
  console.log(`  daarvan gemeld als afwijkend           ${gezien}`);
  console.log(`  overige meldingen (markies/roma)       ${d.afwijkend.length - gezien}`);

  // Bij 1,10 → 1,20 verandert elke prijs boven de ~€0,10. Een enkele nulprijs kan gelijk
  // blijven, dus we eisen niet 100% maar wel bijna alles.
  const drempel = Math.floor(sun.length * 0.99);
  const geslaagd = gezien >= drempel && d.afwijkend.length - gezien === 0;

  console.log('\n' + '═'.repeat(64));
  if (geslaagd) {
    console.log(`✅ ZELFTEST GESLAAGD — de meetlat zag ${gezien} van de ${sun.length} prijswijzigingen`);
    console.log('   en meldde géén valse afwijkingen bij markiezen en Roma (die staan los van de Sunmaster-opslag).');
    console.log('   De meetlat kan dus niet groen geven terwijl er wél iets verandert.');
  } else {
    console.log(`❌ ZELFTEST GEFAALD — ${gezien} van ${sun.length} gezien (minimaal ${drempel} nodig)`);
    console.log('   De meetlat is niet betrouwbaar. Niet gebruiken als bewijs.');
  }
...
function main() {
  ...
  if (VASTLEGGEN) {
    fs.writeFileSync(SNAPSHOT, JSON.stringify(m, null, 1));
  ...
  const oud = JSON.parse(fs.readFileSync(SNAPSHOT, 'utf8'));
  ...
  if (d.afwijkend.length) {
    ...
    for (const a of d.afwijkend.slice(0, 25)) {
      console.log(`  ${a.soort.padEnd(28)} ${a.k}`);
      console.log(`      was ${eur(a.oudV4)}  →  nu ${eur(a.nieuwV4)}${a.verwacht != null ? `  (verwacht ${eur(a.verwacht)})` : ''}${a.nieuwBot !== a.nieuwV4 ? `  BOT ZEGT ${eur(a.nieuwBot)}` : ''}`);
    }
  }

  const rapport = path.join(UIT, 'vergelijking.json');
  fs.writeFileSync(rapport, JSON.stringify({ factor: FACTOR, nulmeting: oud.gemetenOp, nu: m.gemetenOp, ...d }, null, 1));
