#!/usr/bin/env node
// Eenmalig: de 11 Gripp offertes van 2026-06-10 opnieuw aanmaken met de JUISTE (ACCEPTED) RP offerte
// - Verwijdert oude Gripp offerte, maakt nieuwe aan op BESTAANDE relatie (geen dubbele klanten)
...
async function main() {
  ...
  const today = Object.entries(sentLog).filter(([k, v]) => v.processedAt?.startsWith('2026-06-10'));
  console.log('Opnieuw verwerken: ' + today.length + ' offertes\n');

  const itemsData = await rpGet('/contact-service/' + PID + '/backlogs/' + BACKLOG_ID + '/items');

  for (const [name, old] of today) {
    console.log('--- ' + name + ' (oude Gripp offerte ' + old.grippOfferId + ', relatie ' + old.grippCompanyId + ') ---');
    const item = (itemsData?.items || []).find(i => i.summary === name);
    if (!item) { console.log('  SKIP: item niet gevonden in RP'); continue; }
    ...
    // Kies JUISTE doc: ACCEPTED (nieuwste) > SENT > rest
    ...
    console.log('  Juiste doc: #' + chosen.info.quotationNumber + ' [' + chosen.status + ']');

    if (String(chosen.info.quotationNumber) === String(old.rpDocNumber)) {
      console.log('  Zelfde doc als eerder → offerte was al correct, overslaan');
      continue;
    }
    ...
    for (const line of lines) {
      ...
      offerlines.push({ _ordering: ordering++, product: productId, amount: line.units, sellingprice: parseFloat(priceExcl.toFixed(2)), discount: discountPct, buyingprice: 0, invoicebasis: 1, vat: 27, unit: 3, convertto: 1, rowtype: 1, description: specLines || lineDesc });
    }

    // Verwijder oude Gripp offerte
    const [del] = await gripp([{ method: 'offer.delete', params: { id: old.grippOfferId }, id: 1 }]);
    console.log('  Oude offerte ' + old.grippOfferId + ' verwijderd: ' + (del?.result === true || del?.result?.success !== false ? 'OK' : JSON.stringify(del).substring(0, 80)));

    // Maak nieuwe offerte op BESTAANDE relatie
    ...
    if (discountName) beschrijving.push('Korting: ' + discountName);
    ...
    if (!offerId) { console.log('  ERROR nieuwe offerte: ' + JSON.stringify(createOffer?.error)?.substring(0, 100)); continue; }
    console.log('  Nieuwe Gripp offerte: ' + offerId + ' (€' + (lines.reduce((s, l) => s + l.units * l.pricePerUnit, 0) * (1 - discountPct / 100)).toFixed(2) + ' incl)');

    // Update sent log
    sentLog[name] = { ...old, grippOfferId: offerId, rpDocNumber: chosen.info.quotationNumber, redoneAt: new Date().toISOString() };
