...
async function maakOchtend(hist, poging = 0) {
  ...
  const r = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({
      model: 'claude-sonnet-5', max_tokens: 1500, thinking: { type: 'disabled' }, // Sonnet: grappiger dan Haiku (Daimy 21-08). 08-09: thinking UIT + ruimer budget, anders eten de denk-tokens het budget op en komt de tekst half of niet (halve zinnen in de groep, 'generatie mislukt')
  ...
  if (!tekst || tekst.length < 20 || j?.stop_reason === 'max_tokens') { console.error('API-antwoord:', JSON.stringify(j).slice(0, 400)); throw new Error('ochtend-generatie mislukt' + (j?.stop_reason === 'max_tokens' ? ' (afgekapt)' : '')); }
...
async function maakWeetje(hist) {
  ...
  const r = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    headers: { 'x-api-key': APIKEY, 'anthropic-version': '2023-06-01', 'content-type': 'application/json' },
    body: JSON.stringify({
      model: 'claude-sonnet-5', max_tokens: 1500, thinking: { type: 'disabled' }, // 08-09: zie maakOchtend
  ...
  tekst = tekst.replace(/\s*\u2014+\s*/g, ', ');
  tekst = tekst.charAt(0).toLowerCase() + tekst.slice(1);
  if (!tekst || tekst.length < 20 || j?.stop_reason === 'max_tokens') { console.error('API-antwoord:', JSON.stringify(j).slice(0, 400)); throw new Error('weetje-generatie mislukt' + (j?.stop_reason === 'max_tokens' ? ' (afgekapt)' : '')); }
  return { thema, tekst };
}
