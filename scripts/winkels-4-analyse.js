...
const JAAR_TABS_SET = new Set(JAAR_TABS);
...
// ONLINE-curve: winkelbezoekers (65% conversie) zitten vrijwel allemaal <20 km en blazen de dichtbij-conversie op; een nieuwe winkel
// trekt online-leads naar de online-conversie dichtbij Rijswijk. Winkel-inloop bij de nieuwe winkel is een aparte upside (zie inloop).
const kmVanPlaats = {}; for (const p of plaatsen) kmVanPlaats[p.plaats] = p.kmRijswijk;
const curveMeting = CURVE.map((b, i) => { const van = i ? CURVE[i - 1].tot : 0; let L = 0, A = 0, Lw = 0, Aw = 0, Lall = 0, Aall = 0; for (const l of leads) { const d = kmVanPlaats[norm(l.plaats)]; if (d == null || d < van || d >= b.tot) continue; Lall++; if (l.akkoord) Aall++; if (/online/i.test(l.kanaal)) { L++; if (l.akkoord) A++; } else if (/winkel/i.test(l.kanaal)) { Lw++; if (l.akkoord) Aw++; } } return { band: van + '-' + (b.tot > 1000 ? '∞' : b.tot) + ' km', leads: L, akkoord: A, conv: L ? A / L : 0, winkelLeads: Lw, winkelConv: Lw ? Aw / Lw : 0, alleLeads: Lall, alleConv: Lall ? Aall / Lall : 0, curveOud: b.conv }; });
// inloop-upside: winkelleads per jaar bij Rijswijk (<20 km) en hun conversie → verwachte extra akkoorden uit inloop bij een nieuwe winkel (niet in netto)
const inloopJr = leads.filter((l) => JAAR_TABS_SET.has(l.tab) && /winkel/i.test(l.kanaal)); const inloop = { winkelLeadsJr: inloopJr.length, akkoordJr: inloopJr.filter((l) => l.akkoord).length, conv: inloopJr.length ? inloopJr.filter((l) => l.akkoord).length / inloopJr.length : 0 };
...
const jaar = {};
for (const l of leads) { if (!JAAR_TABS.includes(l.tab)) continue; const p = norm(l.plaats); if (RUIS.has(p)) continue; const e = (jaar[p] = jaar[p] || { leads: 0, akkoord: 0, teVer: 0, omzet: 0, winkel: 0, online: 0 }); e.leads++; if (/online/i.test(l.kanaal)) e.online++; if (l.akkoord) { e.akkoord++; e.omzet += l.omzet || 0; } if (l.teVer) e.teVer++; if (/winkel/i.test(l.kanaal)) e.winkel++; }
...
function uplift(k, netwerk) {
  ...
  for (const p of plaatsen) { const j = jaar[p.plaats]; if (!j) continue; const dK = dist(p, k); if (dK > CATCHMENT_KM) continue; const dBest = Math.min(...netwerk.map((n) => dist(p, n))); if (dK >= dBest) continue; const huidig = p.onlineLeads >= 10 ? p.onlineAkkoord / p.onlineLeads : curveAt(dBest); const nieuw = curveAt(dK); const up = Math.max(0, nieuw - huidig); const e = Math.max(0, j.online - j.teVer) * up + j.teVer * nieuw; extra += e; jaarLeads += j.leads; teVer += j.teVer; huidigAkk += j.online * huidig; catch_.push({ plaats: p.plaats, km: Math.round(dK), kmBestaand: Math.round(dBest), jaarLeads: j.leads, online: j.online, teVer: j.teVer, huidigeConv: r1(100 * huidig), nieuweConv: r1(100 * nieuw), gemeten: p.onlineLeads >= 10, extraAkkoord: r1(e) }); }
...
// score = alleen klantwaarde (Daimy 10-09: magazijn telt niet, het gaat om omzet en winst): 45% bewezen uplift, 25% koopwoningen binnen 15 km, 15% onbenut potentieel, 15% lookalike
function composite(s, max) { const a = s.uplift.extraAkkoordVol / max.extra, b = s.uplift.koopBinnen15 / max.koop, c = (s.lookalikeKlant ? s.lookalikeKlant.score : 50) / 100, e = s.uplift.latentLeads / max.latent; return r1(100 * (0.45 * a + 0.25 * b + 0.15 * e + 0.15 * c)); }
function econ(s) { const real = s.uplift.extraAkkoordVol * 0.75; const omzet = real * ORDERWAARDE, bw = omzet * MARGE; const k200 = kosten(200, 'zicht'), k250pdv = kosten(250, 'pdv'), k175 = kosten(175, 'zicht'), k175l = kosten(175, 'zicht', 'laag'); const inloopBw = (f) => f * inloop.akkoordJr * ORDERWAARDE * MARGE; return { inloopAkk25: Math.round(0.25 * inloop.akkoordJr), inloopAkk50: Math.round(0.5 * inloop.akkoordJr), nettoInloop25: Math.round(bw + inloopBw(0.25) - k200.totaal), nettoInloop50: Math.round(bw + inloopBw(0.5) - k200.totaal), nettoInloop25_175laag: Math.round(bw + inloopBw(0.25) - k175l.totaal), realistischAkkoord: Math.round(real), extraOmzet: Math.round(omzet), brutowinst: Math.round(bw), nettoZicht200: Math.round(bw - k200.totaal), nettoPdv250: Math.round(bw - k250pdv.totaal), nettoZicht175: Math.round(bw - k175.totaal), nettoZicht175laag: Math.round(bw - k175l.totaal), breakEven175laag: breakEven(k175l), breakEvenAkkoord: breakEven(k200), conservatief: Math.round(s.uplift.extraAkkoordVol * 0.5 * ORDERWAARDE * MARGE - k200.totaal), vol: Math.round(s.uplift.extraAkkoordVol * ORDERWAARDE * MARGE - k200.totaal) }; }
...
function evalSet(namen, label) { let net = [RIJSWIJK]; const winkels = []; for (const n of namen) { const k = kandidaten.find((x) => x.naam === n); if (!k) { console.error('scenario: kandidaat niet gevonden', n); continue; } const s = scoreKandidaat(k, net); s.score = composite(s, max0); s.econ = econ(s); winkels.push({ naam: s.naam, gemeente: s.gemeente, lat: s.lat, lon: s.lon, kmRijswijk: Math.round(s.km), kmBerkel: s.kmBerkel, extraAkkoord: s.uplift.extraAkkoordVol, jaarLeadsCatchment: s.uplift.jaarLeadsCatchment, latentLeads: s.uplift.latentLeads, koopIndex15: s.uplift.koopIndex15, koopBinnen15: s.uplift.koopBinnen15, lookalike: s.lookalikeKlant?.score, score: s.score, econ: s.econ, gevoeligheid: gevoeligheid(s), topCatchment: s.topCatchment.slice(0, 5), profiel: s.profiel, lookalikeDetail: s.lookalikeKlant, catchmentVolledig: uplift(k, net.slice(0, net.length)).catchment, wijken15: wijken.filter((w) => dist(w, k) <= 15 && w.koopwoningen).map((w) => ({ naam: w.gemeente + ' – ' + w.naam, km: r1(dist(w, k)), hh: w.hh, koop: w.koop, koopwoningen: w.koopwoningen, inkStd: w.inkStd, woz: w.woz, eengezins: w.eengezins, index: r1(100 * penIndex(w)), kmBestaand: r1(Math.min(...net.map((n) => dist(w, n)))) })).sort((a, b) => b.koopwoningen - a.koopwoningen) }); net = [...net, k]; } const tot = (f) => winkels.reduce((a, w) => a + f(w), 0); return { label, winkels, totaal: { nettoInloop25: tot((w) => w.econ.nettoInloop25), nettoInloop50: tot((w) => w.econ.nettoInloop50), extraAkkoord: tot((w) => w.extraAkkoord), realistischAkkoord: tot((w) => w.econ.realistischAkkoord), extraOmzet: tot((w) => w.econ.extraOmzet), nettoZicht200: tot((w) => w.econ.nettoZicht200), nettoZicht175laag: tot((w) => w.econ.nettoZicht175laag), conservatief: tot((w) => w.econ.conservatief), latentLeads: tot((w) => w.latentLeads) } }; }
const scenarios = [
  ...
  evalSet(['Rotterdam – Feijenoord', 'Amsterdam – De Aker', 'Utrecht – Wijk 04 Noordoost', 'Dordrecht – Wijk 08 Crabbehof/Zuidhoven'], 'J. Rotterdam + Amsterdam-West + Utrecht + Drechtsteden'),
  evalSet(['Rotterdam – Feijenoord', 'Amsterdam – De Aker', 'Dordrecht – Wijk 08 Crabbehof/Zuidhoven', 'Diemen'], 'K. Rotterdam + Amsterdam-West + Drechtsteden + Amsterdam-Oost'),
...
console.log('\nSCENARIO\'S:'); for (const sc of scenarios) console.log(sc.label.padEnd(52), '+' + sc.totaal.extraAkkoord + ' akk (real ' + sc.totaal.realistischAkkoord + ')', 'omzet real €' + Math.round(sc.totaal.extraOmzet / 1000) + 'k', 'netto200 €' + Math.round(sc.totaal.nettoZicht200 / 1000) + 'k', 'inloop25 €' + Math.round(sc.totaal.nettoInloop25 / 1000) + 'k', 'netto175laag €' + Math.round(sc.totaal.nettoZicht175laag / 1000) + 'k', 'conserv €' + Math.round(sc.totaal.conservatief / 1000) + 'k', 'latent', sc.totaal.latentLeads, '\n   ', sc.winkels.map((w) => w.naam + ' +' + w.extraAkkoord + ' (netto ' + Math.round(w.econ.nettoZicht200 / 1000) + 'k, inloop25 ' + Math.round(w.econ.nettoInloop25 / 1000) + 'k, worst ' + Math.round(w.gevoeligheid.worst / 1000) + 'k)').join(' | '));
// ---------- 10. verificatie ----------
const verif = { curveBasis: 'online-leads (winkelbezoekers apart)', inloopRijswijk: inloop, leadsTotaal: leads.length, leadsMetPlaats: leads.filter((l) => l.plaats).length, plaatsenGeocodeerd: plaatsen.length, plaatsenMetGemeente: plaatsen.filter((p) => plaatsGm[p.plaats]).length, leadsInJaarbasis: jaarTotaal.leads, jaarAkkoord: jaarTotaal.akkoord, jaarTeVer: jaarTotaal.teVer, jaarTabs: JAAR_TABS, denHaagAlsEenPlaats: (jaar['den haag'] || {}).leads || 0, kandidatenTotaal: kandidaten.length, kandidatenMetCatchment: base.length, penKernLeadsPer1000Koop: r1(penKern), curveMeting, cbsTabel: '85984NED Kerncijfers wijken en buurten 2024 (opgehaald 2026-09-09)', pdok: 'locatieserver v3_1 (opgehaald 2026-09-09)' };
...
console.log('curve ONLINE:', curveMeting.map((m) => m.band + ' ' + r1(100 * m.conv) + '% (n=' + m.leads + ', alle ' + r1(100 * m.alleConv) + '%)').join(' | ')); console.log('inloop Rijswijk/jr', JSON.stringify(inloop));
