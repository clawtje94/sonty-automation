...
function evalSet(namen, label) { let net = [RIJSWIJK]; const winkels = []; for (const n of namen) { const k = kandidaten.find((x) => x.naam === n); if (!k) { console.error('scenario: kandidaat niet gevonden', n); continue; } const s = scoreKandidaat(k, net); s.score = composite(s, max0); s.econ = econ(s); winkels.push({ naam: s.naam, gemeente: s.gemeente, lat: s.lat, lon: s.lon, kmRijswijk: Math.round(s.km), kmBerkel: s.kmBerkel, extraAkkoord: s.uplift.extraAkkoordVol, jaarLeadsCatchment: s.uplift.jaarLeadsCatchment, latentLeads: s.uplift.latentLeads, koopIndex15: s.uplift.koopIndex15, koopBinnen15: s.uplift.koopBinnen15, lookalike: s.lookalikeKlant?.score, score: s.score, econ: s.econ, gevoeligheid: gevoeligheid(s), topCatchment: s.topCatchment.slice(0, 5) }); net = [...net, k]; } const tot = (f) => winkels.reduce((a, w) => a + f(w), 0); return { label, winkels, totaal: { nettoInloop25: tot((w) => w.econ.nettoInloop25), nettoInloop50: tot((w) => w.econ.nettoInloop50), extraAkkoord: tot((w) => w.extraAkkoord), realistischAkkoord: tot((w) => w.econ.realistischAkkoord), extraOmzet: tot((w) => w.econ.extraOmzet), nettoZicht200: tot((w) => w.econ.nettoZicht200), nettoZicht175laag: tot((w) => w.econ.nettoZicht175laag), conservatief: tot((w) => w.econ.conservatief), latentLeads: tot((w) => w.latentLeads) } }; }
const scenarios = [
