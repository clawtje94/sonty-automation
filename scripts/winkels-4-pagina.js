...
const ADVIES = r.scenarios.find((s) => /^J/.test(s.label));
const ZOEK = { 'Rotterdam – Feijenoord': { kop: 'Rotterdam-Zuid/Oost langs de A16', sub: 'IJsselmonde · Feijenoord · Kralingen · Ridderkerk/Barendrecht (Cornelisland-Reijerwaard scoort gelijk)' }, 'Amsterdam – De Aker': { kop: 'Haarlemmermeer / Amsterdam Nieuw-West bij A4-A9', sub: 'Badhoevedorp · Osdorp · De Aker · Hoofddorp-Noord' }, 'Dordrecht – Wijk 08 Crabbehof/Zuidhoven': { kop: 'Dordrecht-Zuid / Drechtsteden aan de A16', sub: 'Zwijndrecht · H.I. Ambacht · Papendrecht · Alblasserdam binnen 10 km' }, 'Utrecht – Wijk 04 Noordoost': { kop: 'Utrecht-rand aan de A2/A27', sub: 'Noordoost · Overvecht · Leidsche Rijn · Maarssen, Nieuwegein, De Bilt binnen 10 km' } };
...
// ---- secties ----
const kaarten = ADVIES.winkels.map((w, i) => { const [rob, cls] = ROBUUST(w); const z = ZOEK[w.naam]; return `<article class="w4-store"><div class="w4-store-top"><span class="w4-rank">${i + 1}</span><span class="pill ${cls}">${rob}</span></div><h3>${esc(z.kop)}</h3><p class="sub">${esc(z.sub)}</p><dl><div><dt>Extra akkoorden/jr</dt><dd>${w.econ.realistischAkkoord} <small>realistisch (vol ${w.extraAkkoord})</small></dd></div><div><dt>Netto/jr zonder inloop</dt><dd class="${w.econ.nettoZicht200 >= 0 ? 'pos' : 'neg'}">${eurK(w.econ.nettoZicht200)} <small>200 m² · 175 m² ${eurK(w.econ.nettoZicht175)} · slechtst ${eurK(w.gevoeligheid.worst)}</small></dd></div><div><dt>Netto/jr met 25% inloop</dt><dd class="${w.econ.nettoInloop25 >= 0 ? 'pos' : 'neg'}">${eurK(w.econ.nettoInloop25)} <small>aanname</small></dd></div><div><dt>Leads/jr binnen 20 km</dt><dd>${nl(w.jaarLeadsCatchment)} <small>onbenut +${nl(w.latentLeads)}</small></dd></div><div><dt>Koopwoning-index</dt><dd>${w.koopIndex15} <small>Rijswijk-kern = 100</small></dd></div><div><dt>Afstand tot andere winkels</dt><dd>${minuten('Showroom Rijswijk', w.naam) ?? '–'} min <small>Rijswijk${i > 0 ? ' · ' + Math.min(...ADVIES.winkels.slice(0, i).map((o) => minuten(o.naam, w.naam) ?? 999)) + ' min dichtstbijzijnde nieuwe winkel' : ''}</small></dd></div></dl><p class="top">Grootste winst: ${w.topCatchment.slice(0, 4).map((c) => esc(cap(c.plaats)) + ' (' + c.jaarLeads + ' leads, ' + nl(c.huidigeConv, 1) + '→' + nl(c.nieuweConv, 1) + '%)').join(', ')}.</p></article>`; }).join('');
...
const rankRows = rank.map((s, i) => `<tr><td class="num">${i + 1}</td><td>${esc(s.naam)}</td><td class="num"><span class="bar" style="--w:${s.score}%"></span>${s.score}</td><td class="num">+${s.uplift.extraAkkoordVol}</td><td class="num">${nl(s.uplift.jaarLeadsCatchment)}</td><td class="num">${nl(s.uplift.koopBinnen15)}</td><td class="num">${s.uplift.koopIndex15}</td><td class="num">${nl(s.uplift.latentLeads)}</td><td class="num">${s.lookalikeKlant ? s.lookalikeKlant.score : '–'}</td><td class="num">${Math.round(s.km)}</td><td class="num ${s.econ.nettoZicht200 >= 0 ? 'pos' : 'neg'}">${eurK(s.econ.nettoZicht200)}</td></tr>`).join('');
...
<section><h2>Fasering</h2><ol class="fase"><li><b>Nu: Rotterdam-Zuid/Oost.</b> Grootste bewezen vraag buiten Haaglanden, positief in elk scenario. Het eerdere loods-onderzoek (Cornelisland/Reijerwaard) ligt in dit zoekgebied.</li><li><b>Na 6 maanden twee dingen meten.</b> Online-conversie binnen 10 km van winkel 1 moet van ~12% richting ${pct(100 * v.curveMeting[0].conv, 0)} gaan (is de curve causaal?), en hoeveel inloop de winkel zelf trekt tegenover Rijswijk (${v.inloopRijswijk.winkelLeadsJr} bezoekers/jr). Klopt het tweede op 25% of meer, dan zijn winkel 3 en 4 verantwoord.</li><li><b>Dan Haarlemmermeer / Amsterdam Nieuw-West</b> aan de A4/A9, bereikbaar voor Haarlem, Hoofddorp, Amstelveen en Amsterdam-West.</li><li><b>Winkel 3 Utrecht-rand (A2/A27) en 4 Dordrecht-Zuid (A16)</b>, 175-200 m² met echte showroom. Zonder inloop licht negatief, met een kwart van de Rijswijkse inloop ruim positief: pas openen als winkel 1 die inloop laat zien, met lokale marketing vanaf dag 1.</li></ol></section>
