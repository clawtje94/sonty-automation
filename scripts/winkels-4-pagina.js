...
const ROBUUST = (w) => w.gevoeligheid.worst > 0 ? ['robuust', 'good'] : w.gevoeligheid.upliftMin20 > 0 ? ['dun', 'warn'] : ['pas na bewijs', 'bad'];
...
const curveRows = r.verificatie.curveMeting.map((m) => `<div class="curve-bar"><div class="fill" style="height:${Math.round(110 * m.conv / 0.265)}px"></div><b>${pct(100 * m.conv)}</b><span>${esc(m.band)}</span><small>n=${nl(m.leads)}</small></div>`).join('');
...
<header class="w4-head"><p class="eyebrow">Sonty · vestigingsonderzoek · ${esc(r.gegenereerd)}</p><h1>Vier winkels naast Rijswijk</h1><p class="lead">Waar 4 winkels van 175-250 m² het meeste opleveren, gerekend op ${nl(v.leadsTotaal)} Sonty-leads (mei 2024 t/m sep 2026), CBS-koopwoningen en -koopkracht per wijk, en echte rijtijden. Advies: <b>open Rotterdam-Zuid/Oost en Haarlemmermeer/Amsterdam-West</b>; winkel 3 en 4 pas als de uplift van de eerste twee in de praktijk klopt.</p></header>

<section class="w4-stores">${kaarten}</section>
<p class="w4-total">Samen (scenario H): <b>+${ADVIES.totaal.realistischAkkoord} akkoorden/jr</b> realistisch · ${eurK(ADVIES.totaal.extraOmzet)} extra omzet · <b>${eurK(ADVIES.totaal.nettoZicht200)} netto/jr</b> na alle winkelkosten · conservatief (50% uplift) ${eurK(ADVIES.totaal.conservatief)}. Eenmalig ca. ${eurK(k.eenmalig)} inrichting per winkel (aanname).</p>
