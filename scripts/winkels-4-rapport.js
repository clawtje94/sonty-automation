...
const H = r.scenarios.find((s) => /^H/.test(s.label)); const v = r.verificatie; const inl = v.inloopRijswijk; const kost = r.constanten.kostenVoorbeeld.zicht200; const be = r.constanten.breakEven; const pen = r.regressiePenetratie;
const ZOEK = { 'Rotterdam – Feijenoord': ['Rotterdam-Zuid/Oost langs de A16', 'IJsselmonde–Feijenoord–Kralingen; Ridderkerk/Barendrecht (Cornelisland-Reijerwaard) scoort vrijwel gelijk'], 'Amsterdam – De Aker': ['Haarlemmermeer / Amsterdam Nieuw-West bij A4-A9', 'Badhoevedorp–Osdorp–De Aker, Hoofddorp aan de andere kant'], 'Utrecht – Wijk 02 Noordwest': ['Utrecht-West aan de A2', 'Leidsche Rijn / Noordwest'], 'Zuidplas': ['Zuidplas / Rotterdam-Noordoost aan de A20', 'Nieuwerkerk aan den IJssel, richting Gouda en Lansingerland'] };
...
Leiden (${min('Showroom Rijswijk', 'Leiden – Bos- en Gasthuisdistrict')} min van Rijswijk) valt af: het steelt vooral van de bestaande showroom en is bij elke gevoeligheid negatief.
...
4. **Winkel 3 en 4 alleen in klein format** (175 m², lage huur, 1 FTE op afspraak, break-even ${be.zicht175} akkoorden) en met lokale marketing: Utrecht-West en Zuidplas/Rotterdam-NO.
